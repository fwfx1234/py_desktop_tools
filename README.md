...
src/
  app/
    ...
    launcher_runtime_coordinator.py  # 启动器/热键/插件生命周期协调
    plugin_surface_coordinator.py    # 插件窗口宿主管理
    ...
    theme/                      # QML 主题令牌
  features/
    api_test/                   # API 测试工具（HTTP/WebSocket/Mock）
    app_launcher/               # 系统应用索引
    clipboard/                  # 剪贴板历史
    download/                   # 下载工具
    image_compress/             # 图片压缩
    json_parser/                # JSON 解析格式化
    packet_capture/             # 抓包工具（演示/占位）
    qml_demo/                   # QML 学习演示
    qr/                         # 二维码生成/识别
    quick_launch/               # 快速启动（项目/脚本动作）
    remote_files/               # 远程文件管理（SSH/SFTP + 终端）
    system/                     # 系统设置、关于页面
tests/                          # pytest 测试套件
```
...
插件 ViewModel 只在插件 Session 活跃或保留期间注入，名称来自 Manifest 的 `contextProperty`。
...
```bash
uv run python -m compileall src     # Python 语法检查
uv run pytest -v -m "not slow"      # 运行测试
uv run app                          # 启动应用实测
```
