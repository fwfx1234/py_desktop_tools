...
class FtpSftpSshClientService:
    ...
    def __init__(
        ...
        *,
        ...
        self._log = get_logger("features.ftp_sftp_ssh_client.service", plugin_id="ftp-sftp-ssh-client")
    ...
    def save_profile(self, payload: dict) -> list[dict]:
        ...
        self._log.info(
            "ftp_sftp_ssh_client.profile.saved",
            "远程连接配置已保存",
    ...
    def delete_profile(self, profile_id: str) -> list[dict]:
        self._log.info("ftp_sftp_ssh_client.profile.delete_requested", "远程连接配置删除请求", profileId=profile_id)
    ...
    def connect(self, profile_id: str) -> RemoteOperationResult:
        ...
        if profile is None:
            self._log.warning("ftp_sftp_ssh_client.connect.profile_missing", "连接配置不存在", profileId=profile_id)
            raise RuntimeError("连接配置不存在")
        self._log.info(
            "ftp_sftp_ssh_client.connect.start",
            "开始连接远程服务器",
            profileId=profile.id,
            protocol=profile.protocol,
            host=profile.host,
        ...
        try:
            ...
            self.pool.close_session(profile.id)
            raise
        ...
        if not remote_root or remote_root == "/":
            try:
                ...
                self._log.warning(
                    "ftp_sftp_ssh_client.connect.home_dir_failed",
                    "读取远程家目录失败，回退到根目录",
                    profileId=profile.id,
                    error=str(exc),
                )
        ...
        try:
            ...
            self._log.warning(
                "ftp_sftp_ssh_client.connect.remote_list_failed",
                "连接成功但远程目录读取失败",
                profileId=profile.id,
                protocol=profile.protocol,
                host=profile.host,
        ...
        self._log.info(
            "ftp_sftp_ssh_client.connect.complete",
            "远程服务器连接完成",
            profileId=profile.id,
            protocol=profile.protocol,
            host=profile.host,
    ...
    def disconnect(self, profile_id: str | None = None) -> str:
        ...
        if not target:
            return ""
        self._log.info("ftp_sftp_ssh_client.disconnect", "断开远程连接", profileId=target)
    ...
    def list_remote(self, profile_id: str, path: str) -> list[dict]:
        ...
        current = path or "/"
        self._log.debug("ftp_sftp_ssh_client.remote.list", "读取远程目录", profileId=profile_id, remotePath=current)
    ...
    def refresh(self, profile_id: str | None, local_path: str, remote_path: str) -> RemoteOperationResult:
        target = self._resolve_id(profile_id)
        self._log.info("ftp_sftp_ssh_client.refresh", "刷新本地和远程目录", profileId=target, localPath=local_path, remotePath=remote_path)
        local_items = self.list_local(local_path)
    ...
    def change_remote_path(self, profile_id: str | None, path: str) -> RemoteOperationResult:
        target = self._resolve_id(profile_id)
        resolved = path or "/"
        self._log.info("ftp_sftp_ssh_client.remote.change_path", "切换远程目录", profileId=target, remotePath=resolved)
    ...
    def upload(self, profile_id: str | None, local_paths: list[str], remote_dir: str) -> list[str]:
        ...
        self._log.info("ftp_sftp_ssh_client.upload.requested", "上传请求", profileId=target, fileCount=len(local_paths), remotePath=remote_dir)
    ...
    def _ensure_remote_dir(self, backend, remote_path: str) -> None:
        try:
            ...
            self._log.debug(
                "ftp_sftp_ssh_client.remote.mkdir_skipped",
                "远程目录创建已跳过(可能已存在)",
                remotePath=remote_path,
                error=str(exc),
            )
    ...
    def download(self, profile_id: str | None, remote_items: list[dict], local_dir: str) -> list[str]:
        target = self._resolve_id(profile_id)
        session = self.pool.require(target)
        self._log.info("ftp_sftp_ssh_client.download.requested", "下载请求", profileId=target, itemCount=len(remote_items), localPath=local_dir)
        transfer_ids: list[str] = []
    ...
    def mkdir_remote(self, profile_id: str | None, path: str) -> None:
        target = self._resolve_id(profile_id)
        session = self.pool.require(target)
        self._log.info("ftp_sftp_ssh_client.remote.mkdir", "创建远程目录", profileId=target, remotePath=path)
    ...
    def rename_remote(self, profile_id: str | None, source: str, new_name: str) -> None:
        ...
        self._log.info("ftp_sftp_ssh_client.remote.rename", "重命名远程项目", profileId=target, remotePath=source, targetPath=renamed)
        session.backend.rename(source, renamed)

    def delete_remote(self, profile_id: str | None, items: list[dict]) -> None:
        target = self._resolve_id(profile_id)
        session = self.pool.require(target)
        self._log.info("ftp_sftp_ssh_client.remote.delete", "删除远程项目", profileId=target, itemCount=len(items))
        for item in items:
            path = str(item.get("path") or "")
            if not path or path == "/":
                continue
            if bool(item.get("isDir")):
                session.backend.delete_dir(path)
            else:
    ...
    def open_terminal(self, profile_id: str | None = None) -> None:
        ...
        if session.profile.protocol != "sftp":
            self._log.warning("ftp_sftp_ssh_client.terminal.unsupported", "当前连接不支持 SSH 终端", profileId=target)
            raise RuntimeError("只有 SFTP 连接支持 SSH 终端")
        self._log.info("ftp_sftp_ssh_client.terminal.open", "打开 SSH 终端", profileId=target, host=session.profile.host)
