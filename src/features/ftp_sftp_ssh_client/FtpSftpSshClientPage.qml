...
Item {
    ...
    readonly property var vm: typeof ftpSftpSshClientVm === "undefined" ? null : ftpSftpSshClientVm
    readonly property var connectionState: vm ? (vm.connectionState || ({})) : ({})
    readonly property var profiles: vm ? (vm.profiles || []) : []
    readonly property var transfers: vm ? (vm.transfers || []) : []
    readonly property var remoteItems: vm ? (vm.remoteItems || []) : []
    readonly property var ftpLog: vm ? (vm.ftpLog || []) : []
    readonly property string activeProfileIdValue: vm ? (vm.activeProfileId || "") : ""
    readonly property string remotePathValue: vm ? (vm.remotePath || "") : ""
    readonly property string statusMessageValue: vm ? (vm.statusMessage || "") : ""
    readonly property bool connected: (connectionState.status || "") === "connected"
    readonly property bool connecting: (connectionState.status || "") === "connecting"
    readonly property bool errored: (connectionState.status || "") === "error"
    readonly property bool sftpConnected: connected && (connectionState.protocol || "") === "sftp"
    ...
    readonly property var sessionList: (root.vm ? root.vm.sessions : []) || []
    ...
    readonly property string activeProtocol: (root.connectionState.protocol || "").toLowerCase()
    ...
    function selectedProfileName() {
        ...
        if (connecting)
            return "连接中"
        if (connected)
            return (root.connectionState.protocol || "").toUpperCase() + " " + (root.connectionState.host || "")
        return "未选择连接"
    }
    ...
    function selectedProfileSubtitle() {
        ...
        if (root.connectionState.host)
            return root.connectionState.host
    ...
    function statusText() {
        if (root.statusMessageValue)
            return root.statusMessageValue
        if (connected)
            return "已连接"
        if (connecting)
            return "连接中"
        ...
        RowLayout {
            ...
            Rectangle {
                ...
                ColumnLayout {
                            ...
                            MouseArea {
                                ...
                                onClicked: function(mouse) {
                                    ...
                                    if (root.sessionExists(modelData.id))
                                        root.vm.setActiveProfile(modelData.id || "")
                                }
                                onDoubleClicked: {
                                    if (root.sessionExists(modelData.id || "")) {
                                        root.vm.setActiveProfile(modelData.id || "")
                                    } else {
                                        root.vm.connectProfile(modelData.id || "")
                                    }
                                }
                            }
                    ...
                    Label {
                        Layout.fillWidth: true
                        text: (root.profiles || []).length + " 个连接  ·  右键查看操作"
                        color: textFaint
                        font.pixelSize: 11
                ...
                Rectangle {
                    ...
                    RowLayout {
                        ...
                        PushButton {
                            ...
                            onClicked: root.vm.connectProfile(root.selectedProfile ? root.selectedProfile.id || "" : "")
                        }
                ...
                SplitView {
                    ...
                    StackLayout {
                        ...
                        currentIndex: {
                            ...
                            var activeId = root.activeProfileIdValue || ""
                        ...
                        Repeater {
                            ...
                            TerminalPane {
                                required property var modelData
                                bridge: root.vm ? root.vm.terminalBridgeForProfile(modelData.profileId || "") : null
                            }
                        }
                    }
                }
                ...
                Rectangle {
                    ...
                    Rectangle {
                        ...
                        RowLayout {
                            ...
                            Label {
                                text: (root.transfers || []).length > 0
                                    ? "(" + (root.transfers || []).length + ")"
                                    : ""
                                color: textFaint
                                font.pixelSize: 12
                            }
                            ...
                            IconButton {
                                iconName: "mdi6.broom"
                                tooltip: "清理已完成"
                                onClicked: root.vm.clearFinishedTransfers()
                            }
                        }
                    ...
                    Rectangle {
                        ...
                        height: 22
                        color: surface
                        visible: !transfersBar.collapsed && (root.transfers || []).length > 0
                    ...
                    ListView {
                        ...
                        anchors.right: parent.right
                        anchors.top: transfersHeader.bottom
                        anchors.topMargin: (root.transfers || []).length > 0 ? 22 : 0
                        anchors.bottom: parent.bottom
                        clip: true
                        visible: !transfersBar.collapsed
                        ...
                        delegate: Rectangle {
                            ...
                            RowLayout {
                                ...
                                IconButton {
                                    ...
                                    onClicked: root.vm.cancelTransfer(modelData.id || "")
                                }
                            }
                        }
    ...
    Popup {
        ...
        contentItem: Column {
            ...
            UiMenuItem {
                ...
                text: "连接"
                onTriggered: {
                    if (root.contextProfile) root.vm.connectProfile(root.contextProfile.id || "")
                    profileMenu.close()
                }
            }
            UiMenuItem {
                ...
                onTriggered: {
                    if (root.contextProfile) root.vm.setActiveProfile(root.contextProfile.id || "")
                    profileMenu.close()
                }
            }
            UiMenuItem {
                ...
                itemEnabled: root.contextProfile && root.sessionExists(root.contextProfile.id || "")
                onTriggered: {
                    if (root.contextProfile) root.vm.disconnectProfile(root.contextProfile.id || "")
                    profileMenu.close()
                }
            }
            ...
            UiMenuItem {
                ...
                text: "删除"
                onTriggered: {
                    if (root.contextProfile) root.vm.deleteProfile(root.contextProfile.id || "")
                    profileMenu.close()
                }
            }
        }
    ...
    Popup {
        ...
        contentItem: Column {
            ...
            UiMenuItem {
                ...
                onTriggered: {
                    ...
                    if (root.contextItem.isDir)
                        root.vm.changeRemotePath(root.contextItem.path || "")
                    else
                        root.vm.downloadFiles([root.contextItem])
            ...
            UiMenuItem {
                ...
                itemEnabled: root.connected
                text: "刷新"
                onTriggered: { root.vm.changeRemotePath(root.remotePathValue); remoteFileMenu.close() }
            }
            UiMenuSeparator { width: remoteFileMenu.width - 8; dark: root.dark }
            UiMenuItem {
                width: remoteFileMenu.width - 8
    ...
    FolderDialog {
        ...
        onAccepted: {
            ...
            root.vm.downloadFilesTo([root.contextItem], url)
        }
    }
    ...
    Dialog {
        ...
        anchors.centerIn: Overlay.overlay
        onAccepted: {
            root.vm.mkdirRemote(mkdirField.text)
            mkdirField.text = ""
        }
    ...
    Dialog {
        ...
        standardButtons: Dialog.Ok | Dialog.Cancel
        anchors.centerIn: Overlay.overlay
        onAccepted: root.vm.renameRemote(targetPath, renameField.text)
    ...
    MessageDialog {
        ...
        onAccepted: root.vm.deleteRemote(root.selectedRemoteItems)
    }
    ...
    component RemoteFilePane: Rectangle {
        ...
        function currentItem() {
            if (fileList.currentIndex < 0 || fileList.currentIndex >= (root.remoteItems || []).length)
                return null
            return root.remoteItems[fileList.currentIndex]
        }
            ...
            Rectangle {
                ...
                RowLayout {
                    ...
                    IconButton {
                        ...
                        onClicked: root.vm.syncRemoteFromTerminal()
                    }

                    Label {
                        text: (root.remoteItems || []).length + " 项"
                        color: textFaint
                        font.pixelSize: 11
                    }
                }
            ...
            Rectangle {
                ...
                RowLayout {
                    ...
                    IconButton {
                        ...
                        tooltip: "返回上级"
                        enabled: pane.enabled && root.remotePathValue !== "/"
                        onClicked: {
                            var p = String(root.remotePathValue || "/")
                            var idx = p.lastIndexOf("/")
                            var parent = idx <= 0 ? "/" : p.substring(0, idx)
                            root.vm.changeRemotePath(parent || "/")
                        }
                    }
                    ...
                    IconButton {
                        ...
                        onClicked: root.vm.changeRemotePath("")
                    }

                    UiTextField {
                        id: pathField
                        dark: root.dark
                        text: root.remotePathValue
                        Layout.fillWidth: true
                        onAccepted: root.vm.changeRemotePath(text)
                    }
                    ...
                    IconButton {
                        ...
                        tooltip: "跳转"
                        enabled: pane.enabled
                        onClicked: root.vm.changeRemotePath(pathField.text)
                    }
            ...
            Item {
                ...
                ListView {
                    ...
                    delegate: Rectangle {
                        ...
                        MouseArea {
                            ...
                            onDoubleClicked: {
                                if (modelData.isDir)
                                    root.vm.changeRemotePath(modelData.path || "")
                                else
                                    root.vm.downloadFiles([modelData])
                            }
                        }
                    }
                    ...
                    Label {
                        anchors.centerIn: parent
                        visible: pane.enabled && (root.remoteItems || []).length === 0
                ...
                Rectangle {
                    ...
                    ColumnLayout {
                        ...
                        Label {
                            Layout.alignment: Qt.AlignHCenter
                            text: "释放以上传到 " + root.remotePathValue
                            color: selectedBg
                            font.pixelSize: 13
                            font.weight: Font.DemiBold
                        }
            ...
            Rectangle {
                ...
                RowLayout {
                    ...
                    IconButton {
                        ...
                        onClicked: root.vm.openTerminal()
                    }
                    ...
                    IconButton {
                        ...
                        tooltip: "关闭终端"
                        enabled: root.sftpConnected
                        onClicked: root.vm.closeTerminal()
                    }
                }
            ...
            Rectangle {
                ...
                RowLayout {
                    ...
                    PushButton {
                        label: "清空"
                        iconName: "mdi6.broom"
                        enabled: (root.ftpLog || []).length > 0
                        onClicked: root.vm.clearFtpLog()
                    }
                }
