...
def _logger():
    global _LOG
    if _LOG is None:
        _LOG = get_logger("features.ftp_sftp_ssh_client.transfer_service", plugin_id="ftp-sftp-ssh-client")
    return _LOG
...
class RemoteTransferService:
    ...
    def cancel(self, transfer_id: str) -> None:
        _logger().info("ftp_sftp_ssh_client.transfer.cancel", "取消传输", transferId=transfer_id)
        with self._lock:
    ...
    def clear_finished(self) -> None:
        _logger().info("ftp_sftp_ssh_client.transfer.clear_finished", "清理已完成传输")
        with self._lock:
            self._items = {
                item_id: item
    ...
    def close(self) -> None:
        _logger().info("ftp_sftp_ssh_client.transfer.close", "关闭传输服务")
    ...
    def _start(self, direction: TransferDirection, local_path: str, remote_path: str, size: int) -> str:
        ...
        _logger().info(
            "ftp_sftp_ssh_client.transfer.queued",
            "传输任务已加入队列",
    ...
    def _run_transfer(self, handle: TaskHandle, transfer_id: str) -> None:
        ...
        with self._lock:
            ...
            item.message = "传输中"
        _logger().info(
            "ftp_sftp_ssh_client.transfer.running",
            "传输任务开始",
            transferId=transfer_id,
        ...
        try:
            ...
            with self._lock:
                ...
                if item is not None:
                    item.status = "cancelled"
                    item.message = "已取消"
            _logger().info("ftp_sftp_ssh_client.transfer.cancelled", "传输任务已取消", transferId=transfer_id)
            self._emit()
            return
        ...
        with self._lock:
            ...
            if item is not None:
                ...
                item.speed = "0 KB/s"
                item.message = "已完成"
        _logger().info("ftp_sftp_ssh_client.transfer.completed", "传输任务已完成", transferId=transfer_id)
        self._emit()
        self._on_message("传输完成")

    def _mark_failed(self, transfer_id: str, exc: BaseException) -> None:
        _logger().error("ftp_sftp_ssh_client.transfer.failed", "传输任务失败", transferId=transfer_id, error=str(exc))
        with self._lock:
            item = self._items.get(transfer_id)
