...
def _logger():
    global _LOG
    if _LOG is None:
        _LOG = get_logger("features.ftp_sftp_ssh_client.backends", plugin_id="ftp-sftp-ssh-client")
    return _LOG
...
class SftpBackend:
    ...
    def connect(self) -> None:
        ...
        if self.profile.jump_enabled:
            _logger().info(
                "ftp_sftp_ssh_client.sftp.jump.connect_start",
                "开始连接 SFTP 跳板机",
            ...
            if jump_transport is None:
                _logger().error("ftp_sftp_ssh_client.sftp.jump.transport_missing", "跳板机 SSH transport 不可用", host=self.profile.jump_host)
            ...
            _logger().info(
                "ftp_sftp_ssh_client.sftp.jump.connect_complete",
                "SFTP 跳板机连接完成",
                host=self.profile.jump_host,
        ...
        _logger().info(
            "ftp_sftp_ssh_client.sftp.connect_start",
            "开始连接 SFTP 目标服务器",
        ...
        if self._transport is None:
            _logger().error("ftp_sftp_ssh_client.sftp.transport_missing", "SSH transport 不可用", host=self.profile.host)
            raise RuntimeError("SSH transport 不可用")
        _logger().info("ftp_sftp_ssh_client.sftp.connect_complete", "SFTP 目标服务器连接完成", host=self.profile.host, port=self.profile.port)

    def close(self) -> None:
        _logger().info("ftp_sftp_ssh_client.sftp.close", "关闭 SFTP 连接", host=self.profile.host)
    ...
    def list_dir(self, path: str) -> list[RemoteFileItem]:
        _logger().debug("ftp_sftp_ssh_client.sftp.list_dir", "读取 SFTP 目录", host=self.profile.host, remotePath=path)
    ...
    def mkdir(self, path: str) -> None:
        _logger().info("ftp_sftp_ssh_client.sftp.mkdir", "创建 SFTP 目录", host=self.profile.host, remotePath=path)
        sftp = self._open_sftp()
        try:
            sftp.mkdir(path)
        finally:
    ...
    def rename(self, source: str, target: str) -> None:
        _logger().info("ftp_sftp_ssh_client.sftp.rename", "重命名 SFTP 项目", host=self.profile.host, remotePath=source, targetPath=target)
        sftp = self._open_sftp()
        try:
    ...
    def delete_file(self, path: str) -> None:
        _logger().info("ftp_sftp_ssh_client.sftp.delete_file", "删除 SFTP 文件", host=self.profile.host, remotePath=path)
        sftp = self._open_sftp()
        try:
            sftp.remove(path)
        finally:
    ...
    def delete_dir(self, path: str) -> None:
        _logger().info("ftp_sftp_ssh_client.sftp.delete_dir", "删除 SFTP 目录", host=self.profile.host, remotePath=path)
        sftp = self._open_sftp()
        try:
            sftp.rmdir(path)
        finally:
    ...
    def upload_file(self, local_path: str, remote_path: str, progress: ProgressCallback | None = None) -> None:
        _logger().info("ftp_sftp_ssh_client.sftp.upload", "上传 SFTP 文件", host=self.profile.host, localPath=local_path, remotePath=remote_path)
        sftp = self._open_sftp()
        try:
    ...
    def download_file(self, remote_path: str, local_path: str, progress: ProgressCallback | None = None) -> None:
        _logger().info("ftp_sftp_ssh_client.sftp.download", "下载 SFTP 文件", host=self.profile.host, remotePath=remote_path, localPath=local_path)
    ...
    def open_terminal(self):
        ...
        _logger().info("ftp_sftp_ssh_client.sftp.terminal.open", "打开 SFTP 共享 SSH 终端", host=self.profile.host)
...
class FtpBackend:
    ...
    def connect(self) -> None:
        ...
        with self._lock:
            ...
            _logger().info(
                "ftp_sftp_ssh_client.ftp.connect_start",
                "开始连接 FTP/FTPS 服务器",
            ...
            if self.profile.protocol == "ftps" and hasattr(self._ftp, "prot_p"):
                self._ftp.prot_p()
            _logger().info(
                "ftp_sftp_ssh_client.ftp.connect_complete",
                "FTP/FTPS 服务器连接完成",
                protocol=self.profile.protocol,
                host=self.profile.host,
                port=self.profile.port,
    ...
    def close(self) -> None:
        _logger().info("ftp_sftp_ssh_client.ftp.close", "关闭 FTP/FTPS 连接", protocol=self.profile.protocol, host=self.profile.host)
        with self._lock:
            ftp = self._ftp
            self._ftp = None
        if ftp is None:
    ...
    def list_dir(self, path: str) -> list[RemoteFileItem]:
        with self._lock:
            ...
            try:
                _logger().debug("ftp_sftp_ssh_client.ftp.list_mlsd", "使用 MLSD 读取 FTP/FTPS 目录", protocol=self.profile.protocol, host=self.profile.host, remotePath=path)
                ...
                _logger().warning(
                    "ftp_sftp_ssh_client.ftp.list_mlsd_failed",
    ...
    def mkdir(self, path: str) -> None:
        with self._lock:
            _logger().info("ftp_sftp_ssh_client.ftp.mkdir", "创建 FTP/FTPS 目录", protocol=self.profile.protocol, host=self.profile.host, remotePath=path)
            self._require_ftp().mkd(path)

    def rename(self, source: str, target: str) -> None:
        with self._lock:
            _logger().info("ftp_sftp_ssh_client.ftp.rename", "重命名 FTP/FTPS 项目", protocol=self.profile.protocol, host=self.profile.host, remotePath=source, targetPath=target)
            self._require_ftp().rename(source, target)

    def delete_file(self, path: str) -> None:
        with self._lock:
            _logger().info("ftp_sftp_ssh_client.ftp.delete_file", "删除 FTP/FTPS 文件", protocol=self.profile.protocol, host=self.profile.host, remotePath=path)
            self._require_ftp().delete(path)

    def delete_dir(self, path: str) -> None:
        with self._lock:
            _logger().info("ftp_sftp_ssh_client.ftp.delete_dir", "删除 FTP/FTPS 目录", protocol=self.profile.protocol, host=self.profile.host, remotePath=path)
            self._require_ftp().rmd(path)

    def upload_file(self, local_path: str, remote_path: str, progress: ProgressCallback | None = None) -> None:
        with self._lock:
            _logger().info("ftp_sftp_ssh_client.ftp.upload", "上传 FTP/FTPS 文件", protocol=self.profile.protocol, host=self.profile.host, localPath=local_path, remotePath=remote_path)
            ftp = self._require_ftp()
    ...
    def download_file(self, remote_path: str, local_path: str, progress: ProgressCallback | None = None) -> None:
        with self._lock:
            _logger().info("ftp_sftp_ssh_client.ftp.download", "下载 FTP/FTPS 文件", protocol=self.profile.protocol, host=self.profile.host, remotePath=remote_path, localPath=local_path)
            ftp = self._require_ftp()
            Path(local_path).parent.mkdir(parents=True, exist_ok=True)
