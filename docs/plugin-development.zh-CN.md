...
class LauncherContext:
    ...
    clipboard_type: str = ""               # 剪贴板数据类型（text / image / files）
    clipboard_preview: str = ""            # 剪贴板内容预览（前 100 字符）
    clipboard_text: str = ""               # 剪贴板完整文本
    detected_clipboard_kinds: frozenset[str] = ()  # 剪贴板类型
```
...
| `clipboard` | 剪贴板 | window | **background** | 后台服务 + 懒 UI |
